...
# NEW: 添加 --resume 標記
@click.option("--resume", is_flag=True, default=False, help="Resume training from the last checkpoint.")
def main(**kwargs):
    pass
...
@click.option("--memory_bank_type", type=click.Choice(['spatial', 'sampled']), default='spatial', help="Type of memory bank strategy.")
@click.option("--sampling_ratio", type=float, default=0.1, help="Sampling ratio for coreset (only used if type is coreset).")
def net(
        backbone_names,
...
@click.option("--k_shot", type=int, default=0, help="Number of shots for few-shot")
def dataset(
        name,
        data_path,
        aug_path,
    ...
    def get_dataloaders(seed, test, get_name=name, save_root=None):
        dataloaders = []
        for subdataset in subdatasets:
            ...
            support_save_dir = None
            if save_root is not None:
                support_save_dir = os.path.join(save_root, "support_sets")
...
def run(
        ...
        run_name,
        test,
        resume,  # NEW: 接收 resume 參數
):
    methods = {key: item for (key, item) in methods}
    device = utils.set_torch_device(gpu)
    utils.fix_seeds(seed, device)

    # MODIFIED: 根據 resume 標記決定 'mode'
    storage_mode = "overwrite"
    if resume:
        storage_mode = "resume"  # 假設 utils.create_storage_folder 支援 "resume" 模式
        LOGGER.info(f"Resuming training for: {run_name}")
    
    run_save_path = utils.create_storage_folder(
        results_path, log_project, log_group, run_name, mode=storage_mode
    )
    # --- END OF MODIFICATION ---

    date_str = datetime.now().strftime('%m%d')
    list_of_dataloaders = methods["get_dataloaders"](seed, test, save_root = os.path.join(run_save_path, date_str))
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        models_dir = os.path.join(run_save_path, date_str, "models")
        ...
        for i, GLASS in enumerate(glass_list):
            ...
            GLASS.set_model_dir(models_dir, dataset_name)
            if test == 'ckpt':
            ...
            if type(flag) != int:
                i_auroc, i_ap, p_auroc, p_ap, p_pro, epoch = GLASS.tester(dataloaders["training"], dataloaders['testing'], dataset_name)
